...
<div class="container-fluid">
    <p>Внимание, переводы не отображаются, пока они не будут экспортированы обратно в файл app/lang с использованием <code>php artisan translation:export</code> команды или кнопки Опубликовать.</p>
    ...
    <p>
        ...
        <form class="form-import" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postImport') ?>" data-remote="true" role="form">
            ...
            <div class="form-group">
                <div class="row">
                    ...
                    <div class="col-sm-2">
                    <button type="submit" class="btn btn-success btn-block"  data-disable-with="Loading..">Импортировать переводы</button>
                    </div>
                </div>
            </div>
        </form>
        <form class="form-find" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postFind') ?>" data-remote="true" role="form" data-confirm="Are you sure you want to scan you app folder? All found translation keys will be added to the database.">
            <div class="form-group">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <button type="submit" class="btn btn-info" data-disable-with="Searching.." >Найти переводы в файлах</button>
            </div>
        </form>
        <?php endif; ?>
        <?php if(isset($group)) : ?>
    ...
    <form role="form" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAddGroup') ?>">
        <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
        <div class="form-group">
            <p>Выберите группу, чтобы отобразить групповые переводы. Если группы не отображаются, убедитесь, что вы выполнили миграции и импортировали переводы.</p>
        ...
        <div class="form-group">
            <label>Введите имя новой группы и начните редактировать переводы в этой группе</label>
            <input type="text" class="form-control" name="new-group" />
        </div>
        <div class="form-group">
            <input type="submit" class="btn btn-default" name="add-group" value="Добавить и редактировать ключ" />
        </div>
    </form>
    <?php if($group): ?>
        <form action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAdd', array($group)) ?>" method="POST"  role="form">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <div class="form-group">
                <label>Добавить новые ключи в текущую группу</label>
                <textarea class="form-control" rows="3" name="keys" placeholder="Добавьте по 1 ключу в каждой строке без префикса группы"></textarea>
            </div>
            <div class="form-group">
    ...
    <?php else: ?>
            ...
            <form class="form-add-locale" method="POST" role="form" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAddLocale') ?>">
                ...
                <div class="form-group">
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-2">
                            <button type="submit" class="btn btn-default btn-block"  data-disable-with="Adding..">Добавить новую локаль</button>
                        </div>
                    </div>
                </div>
            </form>
        ...
        <fieldset>
            <legend>Экспорт всех переводов</legend>
            <form class="form-inline form-publish-all" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postPublish', '*') ?>" data-remote="true" role="form" data-confirm="Are you sure you want to publish all translations group? This will overwrite existing language files.">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <button type="submit" class="btn btn-primary" data-disable-with="Publishing.." >Опубликовать всё</button>
            </form>
        </fieldset>
